...
function isPolishHoliday(date: Date): boolean {
  ...
  /** Fixed holidays */
  if (month === 0 && dayOfMonth === 1) return true; // Nowy Rok
  if (month === 0 && dayOfMonth === 6) return true; // Trzech Króli (Objawienie Pańskie)
  if (month === 4 && dayOfMonth === 1) return true; // Święto Pracy
  if (month === 4 && dayOfMonth === 3) return true; // Święto Konstytucji Trzeciego Maja
  if (month === 7 && dayOfMonth === 15) return true; // Wniebowzięcie Najświętszej Maryi Panny
  if (month === 10 && dayOfMonth === 1) return true; // Wszystkich Świętych
  if (month === 10 && dayOfMonth === 11) return true; // Święto Niepodległości
  if (month === 11 && dayOfMonth === 24) return true; // Wigilia Bożego Narodzenia
  if (month === 11 && dayOfMonth === 25) return true; // Boże Narodzenie (I dzień)
  if (month === 11 && dayOfMonth === 26) return true; // Boże Narodzenie (II dzień)
